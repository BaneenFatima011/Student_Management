...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['approved'])) {
    ...
    $approved = implode(', ', $approved_ids);

    $update_sql = "UPDATE CREDENTIALS SET STATUS = 'Approved' WHERE ID IN ($approved)";
...
$sql = "SELECT STATUS, NAME, EMAIL, ID, ROLE FROM CREDENTIALS WHERE ROLE != 'Admin' AND STATUS!='Approved'";
$result = mysqli_query($connection, $sql);
?>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap"><input type="checkbox" name="approved[]" value="<?php echo $row['ID']; ?>"></td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
            <button type="submit" class="block w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded mt-4">Update Status</button>
        </form>
    </div>
</div>
</body>
